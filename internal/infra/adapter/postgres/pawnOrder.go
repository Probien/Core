...
func (r *PawnOrderRepositoryImpl) GetById(id int) (*dto.PawnOrder, error) {
	...
	if pawnOrder.ID == 0 {
		return nil, adapter.PawnOrderNotFound
	}
...
func (r *PawnOrderRepositoryImpl) GetAll(params url.Values) (*[]dto.PawnOrder, map[string]interface{}, error) {
	...
	if err := r.database.Model(&dto.PawnOrder{}).Scopes(adapter.Paginate(params, paginationResult)).Preload("Customer").Preload("Employee").Preload("Status").Find(&pawnOrders).Error; err != nil {
